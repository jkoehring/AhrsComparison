 ...
 * 
 * Modified by J. Koehring as follows:
 * 	1) Fix gyro sensitivity
 * 	2) Fix byte type promotion errors
 * 	3) Remove use of Gyro interface
 * 	4) Add reset() method.
 */
...
public class ITG3200 extends SensorBase
{
	...
	// Used to compute rate of samples coming from gyro
	private SampleRate measuredSampleRate;
	...
	public ITG3200(Port port, DigitalSource interrupt, boolean altAddress)
	{
		...
		// Set up dynamic sample rate measurement:
		measuredSampleRate = new SampleRate(100);
		measuredSampleRate.start();
	...
	private void readCalibrationAxes()
	{
		...
		measuredSampleRate.addSample();

		numCalibrationSamples++;
		xGyro.calibrationSum += (axes[0] << 8) | (axes[1] & 0xff);
		yGyro.calibrationSum += (axes[2] << 8) | (axes[3] & 0xff);
		zGyro.calibrationSum -= (axes[4] << 8) | (axes[5] & 0xff);
	}
	...
	/**
	 * Resets angles of all gyro axes to 0.
	 */
	public void reset()
	{
		synchronized (this)
	...
	/**
	 * Gets a {@link GyroAxis} object representing the x axis of the gyro.
	 * 
	 * @return the x axis
	 */
	...
	/**
	 * Gets a {@link GyroAxis} object representing the y axis of the gyro.
	 * 
	 * @return the y axis
	 */
	...
	/**
	 * Gets a {@link GyroAxis} object representing the z axis of the gyro.
	 * 
	 * @return the z axis
	 */
	...
	/**
	 * Returns the measured rate of samples coming from the gyro.
	 */
	public double getMeasuredSampleRate()
	{
		return measuredSampleRate.getSampleRate();
	}
}
